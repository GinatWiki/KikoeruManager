...
- 发版产物分两档：**tag 推送只构建 GHCR Docker 镜像（`ghcr.yml`）**；Windows 三件（免安装 console zip、免安装 noconsole zip、Windows 安装包，`.github/workflows/release.yml` 的 `build-windows`）**必须维护者明确要求后手动 workflow_dispatch 触发**（在目标 tag ref 上触发即可，tag 输入留空），不随 tag 自动构建。
