...
### v1.7.6 (2026-07)
- 优化 auto_process 流程：将元数据获取移到解压之前，避免元数据失败导致无效解压
- 新增 voicehub.top 作为第三备用元数据源（DLsite → asmr.one → voicehub.top）

### v1.7.5 (2026-04)
- 修复7位RJ号规范化丢失前导零的问题（如 1599822 → RJ01599822）
- 修复RAR文件因 -mcp=65001 导致的 E_INVALIDARG 错误（改为 fallback 方式）
...
- 修复"已存在文件夹处理"页面无法框选多个文件夹的问题（添加 Shift+点击范围选择）
- 问题作品页面添加"批量删除"按钮
- 修复中文密码解压失败的问题（7z 添加 -mcp=65001 强制 UTF-8 编码）
- 后端 SKIP 和 DELETE 操作分离（跳过不删除文件，删除实际删除文件）
...
- 修复 p7zip-full 不支持 RAR5 压缩格式的问题
